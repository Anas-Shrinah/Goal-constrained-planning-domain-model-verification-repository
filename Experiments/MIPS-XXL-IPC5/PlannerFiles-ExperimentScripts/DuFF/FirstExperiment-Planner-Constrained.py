...
def PrepearDomainFile(errorDepth):
	...
	with open("./Experiments/FirstExperiment/Planner/Constrained/PDDL/FirstExperiment-Constrained-PlanningDomainModel.pddl", "r") as myFile:
		for num, line in enumerate(myFile, 1):
			if ":precondition (and (= (critical-variable)" in line:
				print 'found at line:', num
				break
	with open('./Experiments/FirstExperiment/Planner/Constrained/PDDL/FirstExperiment-Constrained-PlanningDomainModel.pddl', 'r') as file:
	    # read a list of lines into data
	    data = file.readlines()

	data[num-1] = '    :precondition (and (= (critical-variable) '+str(errorDepth)+') )\n'

	with open('./Experiments/FirstExperiment/Planner/Constrained/PDDL/FirstExperiment-Constrained-PlanningDomainModel.pddl', 'w') as file:
...
if __name__ == '__main__':
	...
	for errorDepth in range(1,31):
		...
		with open('./Experiments/FirstExperiment/Planner/Constrained/Results/CompileOut/CompileOut-Constrained-'+str(errorDepth)+'.txt', 'w') as f:
			process = subprocess.call(['./CompileAndPlannerRun-FirstExperiment-Constrained.sh'], stdout=f)

		with open('./Experiments/FirstExperiment/Planner/Constrained/Results/FF-MetricOut/FF-MetricOut-Constrained-'+str(errorDepth)+'.txt', 'w') as f:
